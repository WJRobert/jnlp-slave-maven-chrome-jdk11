...
RUN curl https://copr.fedorainfracloud.org/coprs/alsadi/dumb-init/repo/epel-7/alsadi-dumb-init-epel-7.repo -o /etc/yum.repos.d/alsadi-dumb-init-epel-7.repo && \ 
  ...
  yum $DISABLES --disableplugin=subscription-manager -y --setopt=tsflags=nodocs update && \
  yum $DISABLES --disableplugin=subscription-manager -y --setopt=tsflags=nodocs install wget && \
  wget https://dl.google.com/linux/direct/google-chrome-stable_current_x86_64.rpm && \
  yum $DISABLES --disableplugin=subscription-manager -y install ./google-chrome-stable_current_x86_64.rpm && \
