...
RUN curl https://copr.fedorainfracloud.org/coprs/alsadi/dumb-init/repo/epel-7/alsadi-dumb-init-epel-7.repo -o /etc/yum.repos.d/alsadi-dumb-init-epel-7.repo && \ 
  ...
  yum $DISABLES subscription-manager repos --disable=rhel-fast-datapath -y --setopt=tsflags=nodocs update && \
  yum $DISABLES subscription-manager repos --disable=rhel-fast-datapath -y --setopt=tsflags=nodocs install wget && \
  wget https://dl.google.com/linux/direct/google-chrome-stable_current_x86_64.rpm && \
  yum $DISABLES subscription-manager repos --disable=rhel-fast-datapath -y install ./google-chrome-stable_current_x86_64.rpm && \
  ...
  yum $DISABLES subscription-manager repos --disable=rhel-fast-datapath -y install java-11-openjdk-devel
